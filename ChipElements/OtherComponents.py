...
def create_test_junction_nb_electrode(pad_length=400, pad_width=175, pad_space=50):
    points_test_junction_nb_electrodes = [(-pad_width-pad_space/2, -pad_length/2), (-pad_space/2, -pad_length/2),
                                          (-pad_space/2, -2), (pad_space/2, -2), (pad_space/2, -pad_length/2),
                                          (pad_width+pad_space/2, -pad_length/2), (pad_width+pad_space/2, pad_length/2),
                                          (pad_space/2, pad_length/2), (pad_space/2, 2), (-pad_space/2, 2),
                                          (-pad_space/2, pad_length/2), (-pad_width-pad_space/2, pad_length/2),
                                          (-pad_width-pad_space/2, -pad_length/2)]
    test_junction_nb_electrode = gdspy.Polygon(points_test_junction_nb_electrodes)
    test_junction_nb_electrode_periphery = gdspy.Rectangle((-pad_width-3*pad_space/2, -pad_length/2-pad_space),
                                                           (pad_width+3*pad_space/2, pad_length/2+pad_space))
    return test_junction_nb_electrode_periphery, test_junction_nb_electrode

# launch_pad_locations = [[-np.pi / 2, -np.pi / 2],
#                         [-1199, 1199],
#                         [2825, 2825]]
# aaa, test_polygon = create_launch_pad(locations=launch_pad_locations)
#
# cell_test = gdspy.Cell('cell_test')
# cell_test.add(test_polygon)
# # gdspy.write_gds('test_PTC_resonator.gds', unit=1.0e-6, precision=1.0e-9)
# # gdspy.write_gds('test_readout_resonator.gds', unit=1.0e-6, precision=1.0e-9)
# gdspy.LayoutViewer()
